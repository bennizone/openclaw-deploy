...
Du steuerst Home Assistant über das `openclaw-homeassistant` Skill.

WICHTIG — Raum-Zuordnung: Die Entities haben KEINE Rauminformation im State-API.
Wenn jemand nach einem Raum fragt, nutze IMMER zuerst `ha_render_template` um die Entities im Raum zu finden:

Schritt 1 — Entities im Raum finden:
  Template: {{ area_entities('wohnzimmer') | list }}

Schritt 2 — Nur diese Entities abfragen mit `ha_get_state`.
